...
**注意**：V3 CFG 将负面和正面提示词批量合并为一次 transformer 调用（batch_size=2），比 V3 CP 的串行 CFG 快约 6%。其他版本（V1/V2/Flash）的 CFG 在每步中连续运行两次 transformer。
...
**为什么 V3 CFG 比 V3 CP 快**：两者都使用 TP=4 加 2 个数据并行 rank。V3 CP 分割序列，需要在每个注意力层进行 K/V all-gather（40 层 × 每步 2 次调用 = 80 次 all-gather）。V3 CFG 分割批次，每个 rank 看到完整序列，**无需 K/V all-gather** — 只在模型入口分散和出口聚合。这节省了约 6% 的总时间。
...
| TRN2 V3 CP（上下文并行 + NKI） | ~0.78s/step | ~64s |
...
**V3 CFG 达到 H100 同等性能！** 通过使用 CFG 并行（DP=2）结合张量并行（TP=4），将负面和正面提示词批量合并，避免了 K/V all-gather 开销。V3 CFG 比 V3 CP 快约 6%（60s vs 64s）。
