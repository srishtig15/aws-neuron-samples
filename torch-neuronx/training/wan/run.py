...
def parse_args():
    parser = argparse.ArgumentParser(
            prog='neuron-wan-training-test-wrapper',
            description='Test wrapper for Neuron Wan training recipe')
    ...
    parser.add_argument('--tensor_parallel_degree', type=int, default=4, help='Tensor parallelism degree (default: 8, must divide world_size)')
    ...
    assert args.training_script_path is not None, "Need to pass the path of the training script via --training_script_path (path to wan_neuron.py)"

    # Build the test name that will get used by the ModuleTester out of the args we parsed
    test_name = f"wan_{args.model}_training-{args.resolution}-batch{args.batch_size}-AdamW-{WORLD_SIZE}w-zero1_optimizer-grad_checkpointing"
